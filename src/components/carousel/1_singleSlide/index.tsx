...
const Carousel = ({ images, initialIndex = 0 }: { images: string[]; initialIndex?: number }) => {
    ...
    const replaceSlideTo = useCallback(
        (nextIndex: number, direction?: Direction) => {
            ...
            const moveTo = getDirection(currentIndex, nextIndex, slideLength, direction)
    ...
    return (
        <>
            ...
            <div className={cx('carousel')}>
                ...
                <button className={cx('moveTrigger', 'moveLeft')} onClick={() => move('left')} />
                <button className={cx('moveTrigger', 'moveRight')} onClick={() => move('right')} />
            </div>
        </>
    )
}
