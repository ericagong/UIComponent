...
const useAutoPlacement = (
    ...
    useLayoutEffect(() => {
        ...
        const hasVerticalSpace = anchorRect.bottom + floatingRect.height < viewportRect.height
        const verticalPlacement: Direction = hasVerticalSpace ? 'top' : 'bottom'

        // 수평 방향: 오른쪽으로 띄워도 충분한 공간이 있으면 오른쪽(left 기준)
        const hasHorizontalSpace = anchorRect.right + floatingRect.width < viewportRect.width
        const horizontalPlacement: Direction = hasHorizontalSpace ? 'left' : 'right'
            ...
            setStyle({
                [verticalPlacement]:
                    verticalPlacement === 'top'
                        ? anchorAbsoluteTop + anchorRect.height + +(preferredPosition.top ?? 0)
                        : viewportRect.height -
                          anchorAbsoluteTop +
                ...
                [horizontalPlacement]:
                    horizontalPlacement === 'left'
                        ...
                        : viewportRect.width - anchorRect.right + +(preferredPosition.right ?? 0),
    ...
    }, [viewportRect, anchorRef, floatingRef, preferredPosition, positionAttribute])
