...
const rectKeys: (keyof Rect)[] = ['left', 'top', 'width', 'height', 'scrollHeight']
...
const createViewportRectGetter = () => {
    ...
    return () => {
        const $scrollingElem = typeof document !== 'undefined' && document.scrollingElement
...
const ViewportRectContextProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    return (
        <ViewportRectContext.Provider value={viewportRect}>{children}</ViewportRectContext.Provider>
    )
}
